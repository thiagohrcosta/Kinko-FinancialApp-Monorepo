...
**SHINO** is a full-stack fintech platform composed of:
...
Instead of storing mutable balances, SHINO operates on a ledger-first accounting model using double-entry bookkeeping. All monetary movements are recorded as immutable entries, and balances are derived mathematically from the ledger.

Stripe is used as the external payment processor, while SHINO maintains its own internal financial accounting engine.
